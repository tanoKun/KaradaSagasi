...
public class SettingStageMenu implements InventoryProvider {
    ...
    public void init(Player player, InventoryContents contents) {
        ...
        contents.set(1, 1, ClickableItem.of(ItemUtils.createItem(Material.WATCH, "§b§l時間変更", Arrays.asList("§7現在: " + stage.getTime()), 1, false), no -> {
        ...
        contents.set(1, 4, ClickableItem.of(ItemUtils.createItem(Material.ENDER_PORTAL_FRAME, "§d§lロケーション関係", 1, false), e -> {
            new SettingStageLocationsMenu(stage).getInv().open(player);
        }));

        contents.set(1, 7, ClickableItem.of(ItemUtils.createItem(Material.MAP, "§f§lその他", 1, false), e -> {

...
public class PrisonLocationDeleteCheckMenu implements InventoryProvider {
    ...
    public PrisonLocationDeleteCheckMenu(Stage stage, TeleportLocation teleportLocation) {
        this.stage = stage;
    ...
    public void init(Player player, InventoryContents contents) {
        contents.set(1, 2, ClickableItem.of(ItemUtils.createItem(Material.WOOL, "§a削除します", 1, false, DyeColor.GREEN), no -> {
            ...
            stage.getPrisonLocationMap().getPrisonTeleportLocations().remove(teleportLocation);
            player.sendMessage(KaradaSagasi.PX + "§b牢屋「" + teleportLocation.getName() + "§b」を削除しました");
            new PrisonLocationsMenu(KaradaSagasi.getPlugin().getStageManager().getStage(stage.getName())).getInv().open(player);
        }));

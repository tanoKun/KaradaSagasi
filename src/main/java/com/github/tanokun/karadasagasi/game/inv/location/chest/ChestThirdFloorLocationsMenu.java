...
public class ChestThirdFloorLocationsMenu implements InventoryProvider {
    private final Stage stage;

    public SmartInventory getInv(){
        return SmartInventory.builder()
                .id("settingStageMenu")
                .title("§c§lステージ >> " + stage.getName() + " >> §d§lロケーション関係 >> §6§lチェスト >> §6§l三階")
                .update(false)
                .updatePeriod(0)
                .provider(this)
                .cancelable(true)
    ...
    public ChestThirdFloorLocationsMenu(Stage stage) {
        this.stage = stage;
    }
    ...
    public void init(Player player, InventoryContents contents) {
        for (Location location : stage.getChestLocationMap().getThirdFloorChestLocations()) {
            contents.add(ClickableItem.of(ItemUtils.createItem(Material.CHEST, "§7Location: " + YamlUtils.LocationToString(location), 1, false), no -> {
                new ChestLocationDeleteCheckMenu(stage, location, 3).getInv().open(player);
            }));
        }

        contents.fillRow(4, ClickableItem.empty(ItemUtils.createItem(Material.STAINED_GLASS_PANE, "  ", 1, false, DyeColor.MAGENTA)));

        contents.set(5, 4, ClickableItem.of(ItemUtils.createItem(Material.ANVIL, "§6§l三階のチェストを作成", 1, false), e -> {
            contents.inventory().close(player);
            new BreakBlockHandle(Material.CHEST, block -> {
                if (stage.getChestLocationMap().getThirdFloorChestLocations().contains(block.getLocation())) {
                    player.sendMessage(KaradaSagasi.PX + "そのチェストは既に追加済みです");
                    KaradaSagasi.playSound(player, Sound.BLOCK_NOTE_BASS, 1 ,1);
                    return;
                }

                stage.getChestLocationMap().getThirdFloorChestLocations().add(block.getLocation());
                player.sendMessage(KaradaSagasi.PX + "§b三階のチェストに「" + YamlUtils.LocationToString(block.getLocation()) + "」を追加しました");

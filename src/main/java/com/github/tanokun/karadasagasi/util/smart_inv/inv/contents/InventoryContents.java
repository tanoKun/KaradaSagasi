...
public interface InventoryContents
{
    ...
    class Impl implements InventoryContents
    {
        ...
        @Override
        public InventoryContents clear() {
            if (!inv.getManager().getOpenedPlayers(inv).contains(player)) return this;
            ItemStack air = new ItemStack(Material.AIR);
            Inventory topInventory = player.getOpenInventory().getTopInventory();
            for (int i = 0; i < inv.getRows() * 9; i++) {
                topInventory.setItem(i, air);
            }
            contents = new ClickableItem[inv.getRows()][inv.getColumns()];
            return this;
        }
